# TunableControls
Runtime-tunable versions of WPILib's PIDController and ProfiledPIDController using AdvantageKit's LoggedTunableNumber. It exposes any constant that you might want to tune.
...
<details>
  ...
  _Yes, calling these constants is technically incorrect because they can be changed, and so are not constant. However, I do not care._
</details>
...
```
This will automatically publish all these values to the "Tuning" NetworkTable, which can then be tuned in AdvantageScope or from another dashboard.
